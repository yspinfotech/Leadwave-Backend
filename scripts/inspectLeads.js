...
(async () => {
  try {
    ...
    console.log('Distinct companyId values (up to 50):', distinctCompanyIds.slice(0,50));

    const samples = await Lead.find().limit(10).lean();
    console.log(`Sample ${samples.length} lead docs (showing id, companyId type/value, isDeleted, created):`);
    samples.forEach((s, i) => {
      console.log(i+1, {
        _id: String(s._id),
        companyId_type: s.companyId === undefined ? 'undefined' : typeof s.companyId,

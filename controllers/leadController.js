...
exports.managerFilterAllLeads = async (req, res) => {
  try {
    ...
    const validLeadFields = [
      'firstName', 'lastName', 'email', 'phone', 'alt_phone', 
      'leadSource', 'tag', 'platform', 'activity', 'leadStatus',
      'assigned_to', 'assigned_by', 'expectedValue', 'last_contacted_date',
      'next_followup_date', 'star'
    ];
    
    Object.keys(otherFilters).forEach(key => {
      if (validLeadFields.includes(key) && otherFilters[key] !== undefined && otherFilters[key] !== '') {
    ...
    console.log('Filter for manager all leads:', JSON.stringify(filter, null, 2));
    ...
    console.log(`Found ${leads.length} leads for manager ${userId} across ${campaigns.length} campaigns`);

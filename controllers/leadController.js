...
exports.filterLeads = async (req, res) => {
  try {
    ...
    console.log(
      `Found ${total} leads, showing ${leads.length} on page ${pageNum}`,
    );
...
exports.filterAllLeads = async (req, res) => {
  try {
    ...
    const { sortBy = "created", sortOrder = "desc", ...filters } = req.query;
...
exports.exportLeads = async (req, res) => {
  try {
    ...
    const { format = "excel", columns = "all", ...filters } = req.query;
    ...
    if (format === "excel") {
      ...
      // Auto-size columns
      const maxWidth = finalData.reduce(
